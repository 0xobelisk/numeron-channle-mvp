...
interface CustomWalletContextProps {
  ...
  signAndExecuteTransaction: (props: SignAndExecuteTransactionProps) => Promise<SuiTransactionBlockResponse>;
  logout: () => void;
...
export const updateGlobalWalletInfo = (info: typeof globalWalletInfo) => {
  globalWalletInfo = { ...info };
};
...
export default function CustomWalletProvider({ children }: { children: React.ReactNode }) {
  ...
  }, [isConnected, isUsingEnoki, address, emailAddress]);
  ...
  //       const sponsorResponse: AxiosResponse<CreateSponsoredTransactionApiResponse> = await axios.post(
  //         '/api/sponsor',
  //         sponsorTxBody,
  //       );
  ...
  //       };
  //       const executeResponse: AxiosResponse<{ digest: string }> = await axios.post(
  //         '/api/execute',
  ...
  const executeTransactionBlockWithoutSponsorship = async ({
    tx,
    options,
  }: ExecuteTransactionBlockWithoutSponsorshipProps): Promise<SuiTransactionBlockResponse> => {
    if (!isConnected) {
      return;
    }
  ...
  const signAndExecuteTransaction = async ({
    ...
    onSuccess,
    onError,
  }: SignAndExecuteTransactionProps): Promise<SuiTransactionBlockResponse> => {
    try {
