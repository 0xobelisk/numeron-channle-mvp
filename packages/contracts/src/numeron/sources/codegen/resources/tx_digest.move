  ...
  const TABLE_NAME: vector<u8> = b"tx_digest";
  ...
  public fun get(dapp_hub: &DappHub): (vector<u8>) {
    let key_tuple = vector::empty();
  ...
  public(package) fun set(dapp_hub: &mut DappHub, value: vector<u8>) {
    let key_tuple = vector::empty();
